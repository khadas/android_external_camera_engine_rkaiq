...
XCamReturn
LensHw::setIrisParams(int position)
{
    ENTER_CAMHW_FUNCTION();
    SmartLock locker (_mutex);
    struct v4l2_control control;
    ...
    LOGD_CAMHW_SUBM(LENS_SUBM,"|||set iris result: %d, control.value %d", position, control.value);
    if (io_control (VIDIOC_S_CTRL, &control) < 0) {
...
XCamReturn
LensHw::getIrisParams(int* position)
{
    ENTER_CAMHW_FUNCTION();
    SmartLock locker (_mutex);
    struct v4l2_control control;
    ...
    *position = control.value;
    LOGD_CAMHW_SUBM(LENS_SUBM,"|||get iris result: %d, control.value %d", *position, control.value);
