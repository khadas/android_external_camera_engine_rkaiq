...
SensorHw::setLinearSensorExposure(RKAiqAecExpInfo_t* expPar)
{
    ...
    XCAM_LOG_DEBUG("%s: frameid: %d, a-gain: %d, time: %d\n",
                   __FUNCTION__,
                   _frame_sequence,
...
SensorHw::setExposureParams(SmartPtr<RkAiqExpParamsProxy>& expPar)
{
    ...
    if (_working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
        XCAM_LOG_DEBUG("%s: a-gain: %d, time: %d\n",
                       __FUNCTION__,
        ...
        XCAM_LOG_DEBUG("%s: lexp: 0x%x-0x%x, mexp: 0x%x-0x%x, sexp: 0x%x-0x%x\n",
                       __FUNCTION__,
...
SensorHw::getEffectiveParams(SmartPtr<RkAiqExpParamsProxy>& expParams, int frame_id)
{
    ...
    if (it == _effecting_exp_map.end()) {
        ...
        if (_effecting_exp_map.size() < 1) {
            ...
            XCAM_LOG_DEBUG("%s: search_id: %d: get 2, lexp: %d from map\n",
                           __FUNCTION__,
                           search_id,
            ...
            XCAM_LOG_DEBUG("%s: search_id: %d: get-last %d, lexp: %d from map\n",
        ...
        XCAM_LOG_DEBUG("%s: search_id: %d: get-find %d, lexp: %d from map\n",
...
SensorHw::handle_sof(int64_t time, int frameid)
{
    ...
    if (frameid - _frame_sequence > 1)
        XCAM_LOG_ERROR("!!!!frame losed,last frameid:%d,current farmeid:%d!!!!\n", _frame_sequence, frameid);

    _frame_sequence = frameid;
    XCAM_LOG_DEBUG("%s: frameid=%d, exp_list size=%d, gain_list size=%d",
    ...
    if(!_exp_list.empty()) {
        exp_time = _last_exp_time = _exp_list.front();
        set_time = true;
        _exp_list.pop_front();
    } else {
    ...
    _mutex.unlock();
#endif
    XCAM_LOG_DEBUG("%s: working_mode=%d,frameid=%d, status: set_time=%d,set_gain=%d\n",
    ...
    if (set_time || set_gain) {
        ...
        if(_working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
            setLinearSensorExposure(ptr_new_exp);
        } else {
            setHdrSensorExposure(ptr_new_exp);
        }
    }
    ...
    if(set_time) {
        ...
        if (_working_mode == RK_AIQ_WORKING_MODE_NORMAL) {
            XCAM_LOG_DEBUG("%s: _effecting_exp_map: add %d, a-gain: %d, time: %d\n",
                           __FUNCTION__, effecting_frame_id,
            ...
            XCAM_LOG_DEBUG("%s: _effecting_exp_map: add %d, lexp: 0x%x-0x%x, mexp: 0x%x-0x%x, sexp: 0x%x-0x%x\n",
                           __FUNCTION__, effecting_frame_id,
...
SensorHw::set_exp_delay_info(int time_delay, int gain_delay)
{
    ...
    XCAM_LOG_DEBUG ("%s _time_delay: %d, _gain_delay:%d\n",
    ...
    if (_time_delay > _gain_delay) {
        ...
        XCAM_LOG_ERROR("Not support gain's delay greater than time's delay!");
