...
AblcResult_t Ablc_Select_Params_By_ISO(AblcParams_t *pParams, AblcParamsSelect_t *pSelect, int isoValue)
{
	...
	printf("%s:(%d) Ablc iso:%d rggb: %d %d %d %d \n", 
...
AblcResult_t AblcConfig(AblcContext_t *pAblcCtx, AblcConfig_t* pAblcConfig)
{
	...
	#if 1
	//xml param
	pAblcConfig->eMode = ABLC_OP_MODE_AUTO;
	#else
	//static init params
	pAblcConfig->eMode = ABLC_OP_MODE_AUTO;
	Ablc_html_params_init(&pAblcConfig->stHtmlParams.stParams);
	#endif
	
	if(pAblcConfig->eMode == ABLC_OP_MODE_AUTO){
		#if 1
		//xml param
		Ablc_xml_params_init(&pAblcCtx->stAuto.stParams, &pAblcConfig->stBlcCalib);
		#else
		memcpy(&pAblcCtx->stAuto.stParams, &pAblcConfig->stHtmlParams.stParams, sizeof(AblcParams_t));
		#endif
		LOGE_ABLC("%s(%d): Ablc %d %d %d %d \n", 
			__FUNCTION__, __LINE__, 
