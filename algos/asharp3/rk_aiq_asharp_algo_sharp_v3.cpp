...
Asharp3_result_t sharp_fix_transfer_V3(RK_SHARP_Params_V3_Select_t *pSelect, RK_SHARP_Fix_V3_t* pFix, float fPercent)
{
	...
	pFix->sharp_sharp_ratio	= (int16_t)ROUND_F(pSelect->sharp_ratio * (1 << rk_sharp_V3_sharp_ratio_fix_bits));	
	pFix->sharp_bf_ratio = (int16_t)ROUND_F(pSelect->bf_ratio * (1 << rk_sharp_V3_bf_ratio_fix_bits));
	pFix->sharp_gaus_ratio = (int16_t)ROUND_F(pSelect->gaus_ratio * (1 << rk_sharp_V3_gaus_ratio_fix_bits));
	pFix->sharp_pbf_ratio = (int16_t)ROUND_F(pSelect->pbf_ratio * (1 << rk_sharp_V3_bf_ratio_fix_bits));

	// SHARP_SHARP_LUMA_DX (0x0008)
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM - 1; i++)
	{
		pFix->sharp_luma_dx[i] = (int16_t)( log((float)(pSelect->luma_point[i+1] - pSelect->luma_point[i])) / log((float)2) );
	}
	...
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM; i++)
	{
		int cur_sigma = FLOOR(pSelect->luma_sigma[i] * pSelect->pbf_gain + pSelect->pbf_add);
	...
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM; i++)
	{
		pFix->sharp_pbf_sigma_inv[i] = (int16_t)ROUND_F((float)1 / (pSelect->luma_sigma[i] * pSelect->pbf_gain + pSelect->pbf_add)  * (1 << sigma_bits[2]));
	}
	...
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM; i++)
	{
		int cur_sigma = FLOOR(pSelect->luma_sigma[i] * pSelect->bf_gain + pSelect->bf_add);
		if(max_val < cur_sigma)
			max_val = cur_sigma;
		if(min_val > cur_sigma)
			min_val = cur_sigma;
	...
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM; i++)
	{
		pFix->sharp_bf_sigma_inv[i] = (int16_t)ROUND_F((float)1 / (pSelect->luma_sigma[i] * pSelect->bf_gain + pSelect->bf_add) * (1 << sigma_bits[2]));
	}

	// SHARP_SHARP_SIGMA_SHIFT (0x00024)
	pFix->sharp_bf_sigma_shift = bf_sigma_shift;
	pFix->sharp_pbf_sigma_shift = pbf_sigma_shift;

	// SHARP_SHARP_EHF_TH_0 (0x0028 -  0x0030)
	// wgt = hf * ehf_th
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM; i++)
	{
		pFix->sharp_ehf_th[i] = pSelect->ehf_th[i];
	}

	// SHARP_SHARP_CLIP_HF_0 (0x0034 -  0x003c)
	for(int i = 0; i < RK_SHARP_V3_LUMA_POINT_NUM; i++)
	{
		pFix->sharp_clip_hf[i] = pSelect->lum_clip_h[i];
	}
	...
	pFix->sharp_pbf_coef[0]	= ROUND_F(pSelect->kernel_pre_bila_filter[0] * (1 << rk_sharp_V3_pbfCoeff_fix_bits));
	pFix->sharp_pbf_coef[1]	= ROUND_F(pSelect->kernel_pre_bila_filter[1] * (1 << rk_sharp_V3_pbfCoeff_fix_bits));
	pFix->sharp_pbf_coef[2]	= ROUND_F(pSelect->kernel_pre_bila_filter[2] * (1 << rk_sharp_V3_pbfCoeff_fix_bits));
	...
	pFix->sharp_pbf_coef[0] = pFix->sharp_pbf_coef[0] + offset;

	// SHARP_SHARP_BF_COEF (0x00044)
	// bf coeff
	// rk_sharp_V3_bfCoeff : [4], [1], [0]
	pFix->sharp_bf_coef[0] = ROUND_F(pSelect->kernel_bila_filter[0] * (1 << rk_sharp_V3_hbfCoeff_fix_bits));
	pFix->sharp_bf_coef[1] = ROUND_F(pSelect->kernel_bila_filter[1] * (1 << rk_sharp_V3_hbfCoeff_fix_bits));
	pFix->sharp_bf_coef[2] = ROUND_F(pSelect->kernel_bila_filter[2] * (1 << rk_sharp_V3_hbfCoeff_fix_bits));
	...
	pFix->sharp_bf_coef[0] = pFix->sharp_bf_coef[0] + offset;

	// SHARP_SHARP_GAUS_COEF (0x00048)
	// rk_sharp_V3_rfCoeff :  [4], [1], [0]
	pFix->sharp_gaus_coef[0] = ROUND_F(pSelect->kernel_range_filter[0] * (1 << rk_sharp_V3_rfCoeff_fix_bits));
	pFix->sharp_gaus_coef[1] = ROUND_F(pSelect->kernel_range_filter[1] * (1 << rk_sharp_V3_rfCoeff_fix_bits));
	pFix->sharp_gaus_coef[2] = ROUND_F(pSelect->kernel_range_filter[2] * (1 << rk_sharp_V3_rfCoeff_fix_bits));
	...
	pFix->sharp_gaus_coef[0] = pFix->sharp_gaus_coef[0] + offset;
