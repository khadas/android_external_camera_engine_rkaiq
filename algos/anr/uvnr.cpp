...
void select_uvnr_params_by_ISO(RKAnr_Uvnr_Params_t *stRKUVNrParams, RKAnr_Uvnr_Params_Select_t *stRKUVNrParamsSelected, int iso)
{
	...
	uvnr_fix_transfer(stRKUVNrParamsSelected);
	#endif
}
...
void uvnr_fix_transfer(RKAnr_Uvnr_Params_Select_t *uvnr)
{
	...
	int i=0;
	rkispp_nr_fix_t  * pNrCfg = (rkispp_nr_fix_t *)malloc(sizeof(rkispp_nr_fix_t));
	...
	//0x00b0
	pNrCfg->uvnr_t2gen_wtp = (unsigned char)(uvnr->bfRatio2 * (1 << RKUVNR_bfRatio));

	//0x00b4
	for(i=0; i<5; i++){
		pNrCfg->uvnr_t2gen_wtq[i] = (unsigned char)(uvnr->kernel_5x5_table[i] * (1 << RKUVNR_kernels));
	}
