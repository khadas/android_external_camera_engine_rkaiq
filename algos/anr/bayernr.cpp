...
{
	int res = RK_AIQ_RET_SUCCESS;
	int i = 0;
	int j = 0;

	printf("%s:(%d) oyyf bayerner xml config start\n", __FUNCTION__, __LINE__);
	if(pParams == NULL){
	...
	for(i=0; i<8; i++){
		pParams->luLevel[i] = pCalibdb->luLevelVal[i];
		printf("luLevel[%d]:%f \n",
			i, pParams->luLevel[i]);
	}
	...
	for(i = 0; i<BAYERNR_MAX_ISO_LEVEL; i++){
		for(j=0; j<8; j++){
			pParams->luRatio[i][j] = pCalibdb->luRatio[j][i];
		}
	}

	for(i = 0; i<BAYERNR_MAX_ISO_LEVEL; i++){
		for(j=0; j<4; j++){
			pParams->w[i][j] = pCalibdb->fixW[j][i];
		}
	}

	pParams->peaknoisesigma = pCalibdb->lamda;
	pParams->sw_rawnr_gauss_en = pCalibdb->gauss_en;
	pParams->rgain_offs = pCalibdb->RGainOff;
	pParams->rgain_filp = pCalibdb->RGainFilp;
	pParams->bgain_offs = pCalibdb->BGainOff;
	pParams->bgain_filp = pCalibdb->BGainFilp;
	pParams->bayernr_edgesoftness = pCalibdb->edgeSoftness;
	pParams->bayernr_gauss_weight0 = pCalibdb->gaussWeight0;
	pParams->bayernr_gauss_weight1 = pCalibdb->gaussWeight1;	

	memcpy(pParams->bayernr_ver_char,  pCalibdb->version, sizeof(pParams->bayernr_ver_char));
	
	printf("%s:(%d) oyyf bayerner xml config end!  ver:%s \n", __FUNCTION__, __LINE__, pParams->bayernr_ver_char);
...
void select_bayernr_params_by_ISO(RKAnr_Bayernr_Params_t *stBayerNrParams, RKAnr_Bayernr_Params_Select_t *stBayerNrParamsSelected, int iso)
{
	...
	bayernr_fix_tranfer(stBayerNrParamsSelected);
	#endif
...
{
	isp2x_rawnr_reg_t * pRawnrCfg = NULL;
	pRawnrCfg = (isp2x_rawnr_reg_t *)malloc(sizeof(isp2x_rawnr_reg_t));
