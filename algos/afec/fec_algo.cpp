...
void SmallMeshCorrect(int imgWidth, int imgHeight, int meshSizeW, int meshSizeH, CameraCoeff camCoeff, double *mapx, double *mapy, unsigned short *pMeshXY)
{
	...
	for (int j = 0; j < meshSizeH; ++j)
	{
		for (int i = 0; i < meshSizeW; ++i)
		{
			...
			// 限制边界
			if (x2 < 0) { x2 = 0; }
			if (x2 > (imgWidth - 1)) { x2 = (imgWidth - 1); }// 1919
			if (y2 < 0) { y2 = 0; }
			if (y2 > (imgHeight - 1)) { y2 = (imgHeight - 1); }// 1079
	...
	unsigned short	*pMeshXY, unsigned short	*pMeshXI, unsigned short	*pMeshYI, unsigned char	*pMeshXF, unsigned char	*pMeshYF)
{
	...
	{
		pMeshXI[i] = *pTmpXi;					pTmpXi += 2;
		pMeshXF[i] = (unsigned char)*pTmpXf;	pTmpXf += 2;
		pMeshYI[i] = *pTmpYi;					pTmpYi += 2;
		pMeshYF[i] = (unsigned char)*pTmpYf;	pTmpYf += 2;
	}

	/* 保存为bin文件 */
	/* MeshXY.bin */
	FILE *fpMeshXY = fopen("MeshXY.bin", "wb");	if (fpMeshXY == NULL) { printf("MeshXY.bin open error!!!");		return; }
	...
	if (fclose(fpMeshXY) != 0) { printf("MeshXY.bin close error!!!"); return; }
	/* MeshXI.bin */
	FILE *fpMeshXI = fopen("MeshXI.bin", "wb");	if (fpMeshXI == NULL) { printf("MeshXI.bin open error!!!");	return; }
	fwrite(pMeshXI, sizeof(unsigned short), meshSizeW * meshSizeH, fpMeshXI);
	if (fclose(fpMeshXI) != 0) { printf("MeshXI.bin close error!!!"); return; }
	/* MeshXF.bin */
	FILE *fpMeshXF = fopen("MeshXF.bin", "wb");	if (fpMeshXF == NULL) { printf("MeshXF.bin open error!!!");	return; }
	fwrite(pMeshXF, sizeof(unsigned char), meshSizeW * meshSizeH, fpMeshXF);
	if (fclose(fpMeshXF) != 0) { printf("MeshXF.bin close error!!!"); return; }
	/* MeshYI.bin */
	FILE *fpMeshYI = fopen("MeshYI.bin", "wb");	if (fpMeshYI == NULL) { printf("MeshYI.bin open error!!!");	return; }
	fwrite(pMeshYI, sizeof(unsigned short), meshSizeW * meshSizeH, fpMeshYI);
	if (fclose(fpMeshYI) != 0) { printf("MeshYI.bin close error!!!"); return; }
	/* MeshYF.bin */
	FILE *fpMeshYF = fopen("MeshYF.bin", "wb");	if (fpMeshYF == NULL) { printf("MeshYF.bin open error!!!");	return; }
	fwrite(pMeshYF, sizeof(unsigned char), meshSizeW * meshSizeH, fpMeshYF);
	if (fclose(fpMeshYF) != 0) { printf("MeshYF.bin close error!!!"); return; }
}
